...
const BASE_SPEEDS = { normal:75, fast:190, tank:48, boss:32 };
const WAVE_SPEED_BONUS = { normal:6, fast:8, tank:3, boss:1.5 };
const FIRE_RATE = { normal:3.0, fast:null, tank:1.8, boss:1.0 };
...
export function updateEnemies(es,playerX,playerY,wave,dt){
  ...
  for(const e of es.enemies){
    ...
    if(e.type==='boss'&&e.hp/e.maxHp<0.5) speed*=1.65;
    ...
    if(wave>=2&&FIRE_RATE[e.type]&&e.fireTimer!==Infinity){
      ...
      if(e.fireTimer<=0){
        ...
        const ESPEED=e.type==='boss'?260:200;
        if(e.type==='boss'){
          const spread=e.hp/e.maxHp<0.5?0.32:0.22;
...
function _drawTank(ctx,r,flash){
  ...
  ctx.beginPath();
  for(let i=0;i<8;i++){const a=(i/8)*Math.PI*2-Math.PI/8;i===0?ctx.moveTo(Math.cos(a)*r,Math.sin(a)*r):ctx.lineTo(Math.cos(a)*r,Math.sin(a)*r);}
  ctx.closePath();
  ...
  ctx.beginPath();
  for(let i=0;i<8;i++){const a=(i/8)*Math.PI*2-Math.PI/8;i===0?ctx.moveTo(Math.cos(a)*r*0.65,Math.sin(a)*r*0.65):ctx.lineTo(Math.cos(a)*r*0.65,Math.sin(a)*r*0.65);}
  ...
  for(let i=0;i<8;i++){const a=(i/8)*Math.PI*2-Math.PI/8;ctx.beginPath();ctx.moveTo(0,0);ctx.lineTo(Math.cos(a)*r*0.65,Math.sin(a)*r*0.65);ctx.stroke();}
  for(let i=0;i<4;i++){
...
function _drawBoss(ctx,r,flash,rotPhase){
  ...
  if(!flash){[-1,0,1].forEach(i=>{ctx.fillStyle='rgba(255,200,210,0.9)';ctx.shadowColor='#ff8899';ctx.shadowBlur=8;ctx.beginPath();ctx.arc(i*r*0.2,-r*0.62,3,0,Math.PI*2);ctx.fill();})}
  ctx.restore();
  ...
  ctx.strokeStyle=flash?'rgba(255,255,255,0.35)':'rgba(255,120,140,0.3)';ctx.lineWidth=1.2;ctx.setLineDash([5,12]);
